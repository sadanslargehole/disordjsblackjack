...
import {deal, dealer_finish, new_deck, value, value_with_hidden, createembed } from '../util/blackjacks'
...
try {
        ...
        const init_dealer_value = value_with_hidden(client.blackjack.dealer_hands.get(authorid)!);
        ...
        if (init_dealer_value == 21){
            message.reply({
                embeds: [createembed(client.blackjack.hands.get(authorid)!, client.blackjack.dealer_hands.get(authorid)!, -1)]
            });
            //TODO: end game
            return
        }
        message.reply({
            embeds: [createembed(client.blackjack.hands.get(authorid)!, client.blackjack.dealer_hands.get(authorid)!,4)]
        }).then((m)=>{
            m.react('🃏');
            m.react('🖐️');
        })
